...
<table cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${tars }" var="tar">
			...
			<td align="center">${tar.id }</td>
			<td align="center">${tar.title }</td>
			<td align="center">${tar.content }</td>
			<td align="center">${tar.author }</td>
			<td align="center">${tar.crtime }</td>
			<td align="center">${tar.cname}</td>
			<td align="center">${tar.isremodTex }</td>
			<td align="center">${tar.ishotTex}</td>
			<td align="center">
			<a href="Tarticleget.do?id=${tar.id}" class="pn-opt">修改</a>
			<a href="Tarticledelete.do?id=${tar.id }" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a>
			</td>
		</tr>
		</c:forEach>
	</tbody>
